...
| 56 | `FICHE` | aucun | dépile un objet et affiche sa fiche (grammaire, § 20.1) : par l'interface si elle sait la montrer, sinon en colonnes |
| 57 | `ÉCRAN_OUVRIR` | constante texte : la description de l'écran | montre l'écran (grammaire, § 22) |
| 58 | `ÉCRAN_LISTE` | élément | dépile une liste ; la montre dans l'écran ouvert, les colonnes par défaut |
| 59 | `ÉCRAN_ATTENDRE` | aucun | valide ce qui précède, attend un événement ; empile l'objet choisi (ou absent), puis le code : 0 fermeture, k + 1 l'élément k |
...
```
en-tête       "GRYM" (4 octets ASCII), version du format : u16 = 26
blocs         nombre : u32, puis pour chacun :
                ...
                fichier source : longueur u32 et octets UTF-8 (vide sauf pour une formule déclarée dans un
                  fichier utilisé, grammaire, § 21 : ses erreurs désignent ce fichier)
