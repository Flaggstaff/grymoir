...
`tant`, `répéter`, `chaque`, `sortir`, `passer`, `selon`, `cas`, `autrement`, `essayer`, `saisir`, `utiliser` (§ 21), `quand`, `ouvrir`, `fermer` (§ 22) (avec `afficher`, `si`, `sinon`, `pour`, `rendre`) commencent des constructions : ils ne peuvent pas commencer le nom d'une action.
...
## 22. Écrans *(A3 : implémenté le 25 septembre 2026)*
...
Implémenté (A3-a) : la déclaration avec liste (recherche, `dont`, `, par`), bouton, texte fixe et titre donné ; `Quand on clique sur « … »` et `Quand on choisit un … dans l'écran …` ; `Ouvrir l'écran …` (programme ou action, hors d'un événement et hors d'`Essayer`) ; `Fermer l'écran.` ; les refus d'analyse du § 22.2 ; le refus de `grym lancer` et `grym servir` ; les deux formes, l'aide à la saisie et la coloration. Implémenté (A3-b) : les zones de saisie (`un pays (texte), « Suisse » au départ, facultatif`, types des entités ; une zone liée à une entité a le menu de ses clés ; fichier et image viendront plus tard) ; `le pays de l'écran`, lu et modifié, dans un événement et dans la liste d'un écran ; `le compositeur choisi de l'écran` (la ligne sélectionnée, ou absent) ; `Quand on change le pays`, `Quand on ouvre l'écran`, `Quand on ferme l'écran`. Une zone validée est lue selon son type ; refusée (« abc » dans un nombre), l'écran le dit et aucun événement ne s'exécute. Implémenté (A3-c) : `Ouvrir la fiche de c.` (`du compositeur`, `de l'œuvre`) ouvre l'écran déduit de l'entité de `c` : un texte par champ (« Titre : Messe »), un bouton par lien (« Compositeur : Bach ») qui ouvre la fiche de l'objet désigné par-dessus, « Modifier » (le formulaire de `Saisir à nouveau`, puis la fiche relue) et « Fermer ». Les champs « plusieurs » n'y figurent pas encore. `Ouvrir`, d'un écran ou d'une fiche, s'emploie aussi dans un événement : le nouvel écran s'ouvre par-dessus, et l'événement reprend à sa fermeture ; `Ouvrir` reste interdit dans `Essayer`. Implémenté (A3-d) : les colonnes choisies, `, avec le titre, le compositeur et la date de naissance du compositeur` (après `dont` et `, par`) : chaque colonne est un champ de l'entité, ou le champ d'un objet qu'elle désigne, lu de droite à gauche ; son titre est ce qu'on a écrit, sans l'article, avec une majuscule ; un lien absent laisse la case vide. Les blocs `côte à côte :` et `l'un sous l'autre :`, qui s'emboîtent et se ferment à l'élément moins indenté. Une ligne qui n'existe plus quand on la choisit ne déclenche rien. En forme compacte : `_avec titre ; compositeur.date_de_naissance`, `_côte_à_côte` … `_fin`, `_l'un_sous_l'autre` … `_fin`.
